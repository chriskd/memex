...
- CLI: uv run --project /srv/fast/code/memex mx (reports version 0.2.0)
...
## Rerun updates (2026-01-25)
### Environment
- Temp project: /srv/fast/devbox.voidlabs.local/tmp/tmp.iQu3VKPCIO
- CLI: uv run --project /srv/fast/code/memex mx (reports version 0.2.0)

### End-to-end journeys (init/add/update/search/relations/publish)
1) Init
   - uv run ... mx init
   - Notes: kb/README.md now has frontmatter; init next-steps use --scope=project.
2) Add
   - uv run ... mx add --title "Alpha Note" --tags test --category guides --content "Alpha content" --scope=project
   - uv run ... mx add --title "Beta Note" --tags test --category guides --content "Linked to Alpha" --relation "guides/alpha-note.md=related" --scope=project
   - uv run ... mx add --title "No Category" --tags test --content "Body" --scope=project -> error: category or directory required unless .kbconfig primary set
3) Update
   - uv run ... mx patch guides/beta-note.md --find "Linked" --replace "Linked v2" (note: patch has no --scope flag)
4) Search
   - uv run ... mx reindex --scope=project -> indexed 3 entries
   - uv run ... mx search "alpha" --scope=project -> **no results** (keyword + min-score 0 also empty)
   - uv run ... mx search "alpha" -> returns alpha/beta entries
   - Gap: --scope=project filter appears broken; search results omit scope metadata (JSON output has no scope)
5) Typed relations
   - uv run ... mx relations guides/beta-note.md --origin relations --json shows edges, but node scope is null
   - relations-add/remove on alpha-note works; frontmatter updates persist
6) Publish
   - uv run ... mx publish -o _site works from .kbconfig project kb
   - Typed relations render in HTML panels

### Perf sanity (project KB)
Timings (single run, seconds):
- reindex: 9.567
- search (mx search "memex"): 1.542
- publish: 0.925
Warnings: none observed

### Docs review deltas
- kb/guides/ai-integration.md and kb/reference/cli.md still reference .kbcontext / mx context init
- kb/reference/cli.md documents unsupported flags (mx add -c, mx search -c) and omits multiple commands
- kb/reference/entry-format.md references mx add --template (flag not present)

### New follow-ups
- m-6c32 Fix mx search --scope=project returning no results
- m-dac5 Docs: remove .kbcontext references and fix CLI flag docs
